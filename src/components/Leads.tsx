...
const Leads: React.FC<LeadsProps> = ({ onOpenChat }) => {
  ...
  const [activeTab, setActiveTab] = useState<'leads' | 'followup'>('leads');
  ...
  const fetchFollowupSettings = async () => {
    try {
      const data = await apiFetch('/api/leads/followup/settings');
      if (data) {
        setFollowupSettings({
          is_enabled: data.is_followup_enabled === 1,
          statuses: JSON.parse(data.followup_statuses || '[]'),
  ...
  const fetchFollowupLogs = async () => {
    try {
      const data = await apiFetch('/api/leads/followup/logs');
  ...
  const saveFollowupSettings = async (newSettings: typeof followupSettings) => {
    setSavingSettings(true);
    try {
      await apiFetch('/api/leads/followup/settings', {
        method: 'POST',
        body: JSON.stringify({
          is_followup_enabled: newSettings.is_enabled ? 1 : 0,
          followup_statuses: JSON.stringify(newSettings.statuses),
          max_followups: newSettings.max_followups
        })
      });
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-gray-100 p-1 rounded-xl mr-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'leads' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Leads
            </button>
            <button
              onClick={() => setActiveTab('followup')}
              className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'followup' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              <Bell size={16} />
              Follow-up
            </button>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-900">Follow-up System Settings</h2>
                  <p className="text-sm text-gray-500">Configure how and when the AI should follow up with leads.</p>
                </div>
              </div>
              <button
                onClick={() => saveFollowupSettings({ ...followupSettings, is_enabled: !followupSettings.is_enabled })}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all ${
                  followupSettings.is_enabled 
                    ? 'bg-green-500 text-white shadow-lg shadow-green-100' 
                    : 'bg-gray-100 text-gray-500'
                }`}
              >
                {followupSettings.is_enabled ? <ToggleRight size={24} /> : <ToggleLeft size={24} />}
                {followupSettings.is_enabled ? 'System Active' : 'System Inactive'}
              </button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <label className="block text-sm font-bold text-gray-700">Target Lead Statuses</label>
                <p className="text-xs text-gray-500">Select which lead statuses should trigger automated follow-ups.</p>
                <div className="flex flex-wrap gap-2">
                  {['New', 'Contacted', 'Qualified', 'Final Customer', 'Not Interested'].map(status => (
                    <button
                      key={status}
                      onClick={() => {
                        const newStatuses = followupSettings.statuses.includes(status)
                          ? followupSettings.statuses.filter(s => s !== status)
                          : [...followupSettings.statuses, status];
                        saveFollowupSettings({ ...followupSettings, statuses: newStatuses });
                      }}
                      className={`px-3 py-2 rounded-xl text-xs font-bold border transition-all ${
                        followupSettings.statuses.includes(status)
              ...
              <div className="space-y-4">
                <label className="block text-sm font-bold text-gray-700">Safety & Limits</label>
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    <div className="flex items-center gap-2">
                      <Clock size={16} className="text-gray-400" />
                      <span className="text-xs font-medium text-gray-700">Message Delay</span>
                    </div>
                    <span className="text-xs font-bold text-indigo-600">30 Seconds</span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    <div className="flex items-center gap-2">
                      <Send size={16} className="text-gray-400" />
                      <span className="text-xs font-medium text-gray-700">Max Follow-ups per Lead</span>
                    </div>
                    <select
                      value={followupSettings.max_followups}
                      onChange={(e) => saveFollowupSettings({ ...followupSettings, max_followups: parseInt(e.target.value) })}
                      className="bg-transparent border-none text-xs font-bold text-indigo-600 focus:ring-0 cursor-pointer"
                    >
                      {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n} Messages</option>)}
                    </select>
                  </div>
                </div>
                <p className="text-[10px] text-gray-400 italic">
                  * 30s delay is enforced to prevent WhatsApp account bans.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Lead</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Message</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Status</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Time</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                    ...
                    followupLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-gray-900">{log.lead_name || 'Unknown'}</span>
                            <span className="text-[10px] text-gray-500">{log.contact_number}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-xs text-gray-600 line-clamp-2 max-w-md">{log.message}</p>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                            log.status === 'sent' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                          }`}>
                            {log.status === 'sent' ? 'DONE' : 'NOT DONE'}
                          </span>
                        </td>
                        <td className="px-6 py-4">

  ...
  return (
    <div className="space-y-12">
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-2xl font-bold hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {[
          { label: 'Leads Generated', value: stats?.totalLeads || 0, growth: '+12.5%', icon: Users, color: 'primary' },
          ...
          >
            <div className={`w-12 h-12 ${item.color === 'primary' ? 'bg-primary/10' : `bg-${item.color}-50`} rounded-2xl flex items-center justify-center mb-4`}>
              <item.icon className={`w-6 h-6 ${item.color === 'primary' ? 'text-primary' : `text-${item.color}-600`}`} />
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#fff', borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
                />
                <Area type="monotone" dataKey="leads" stroke="#25D366" strokeWidth={3} fillOpacity={1} fill="url(#colorGrowth)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
